...
実行するたびに、条件を満たす暗証番号を 1 個表示します。桁数を省略すると 4 桁です。

```sh
passcode-gen            # 4桁（デフォルト）
passcode-gen -d 6       # 6桁
passcode-gen --digits 8 # 8桁
passcode-gen -h         # 使い方（オプション一覧）を表示
```

桁数は `-d` / `--digits` で指定でき、指定できる範囲は **4〜10 桁**です。
桁数は必ずこのフラグで渡してください（`passcode-gen 6` のように位置引数で渡すことはできず、エラーになります）。
